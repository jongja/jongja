...
x = tf.compat.v1.placeholder(dtype = tf.float32, shape = [None, condition])
y = tf.compat.v1.placeholder(dtype = tf.int32, shape = [None, 1]) #for softmax
...
with tf.name_scope("layer1") as scope: ## layer 1  : condition -> 100
    ...
    b1 = tf.Variable(tf.random.normal([70]))
...
train = tf.compat.v1.train.AdamOptimizer(learning_rate=LearningRate).minimize(cost)
...
with tf.compat.v1.Session() as sess:
    ...
    tf.io.write_graph(sess.graph_def, ".", 'Trained_model.pb', as_text=False)
